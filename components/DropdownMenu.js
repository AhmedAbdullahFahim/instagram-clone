...
const DropdownMenu = ({ image, username, id }) => {
  return (
    <Menu>
      <Menu.Button>
        {image ? (
      ...
      >
        {image ? (
          ...
          <Menu.Items className='absolute right-5 top-12 mt-3 w-56 origin-top-right divide-y divide-gray-100 rounded-md bg-white shadow-lg ring-1 ring-black ring-opacity-5 focus:outline-none'>
            <Menu.Item>
              {({ active }) => (
                <div
                  className={`${
